...
(defn- transition-arrows [state svg-el-id app-id state-id {:keys [diagram flow]} states hovered? ommit-arrow2]
  (let [[x1 y1] (live-corner state state-id (:corner diagram))
        w (geom/state-width state-id)
        [x-center y-center] [(+ x1 (/ w 2)) (+ y1 (/ geom/STATE-H 2))]]
    (doall
      (for [[idx [other-state re]] (map-indexed vector flow)
            :when (not= ommit-arrow2 other-state)]
        (let [w-other (geom/state-width other-state)
              [ox oy] (live-corner state other-state (get-in states [other-state :diagram :corner] [0 0]))
              [[ax1 ay1] [ax2 ay2]] (geom/compute-entry&exit {:x x1 :y y1 :h geom/STATE-H :w w}
                                                              {:x ox :y oy :h geom/STATE-H :w w-other})
...
(defn svg-comp [editables ready width height app-id watch-instance]
  (let [state (inter/new-state)
        ...
        wheel-handler (fn [e]
                        ;; preventDefault SIEMPRE, no solo en la rama de zoom: si el
                        ;; evento wheel del pan (dos dedos) queda sin cancelar, Chrome/
                        ;; Safari en Mac interpretan el deltaX no consumido como gesto de
                        ;; navegacion (swipe atras/adelante) y sacan al usuario de la
                        ;; pagina al llegar al borde del contenido.
                        (.preventDefault e)
    ...
    (fn [editables ready width height app-id watch-instance]
      ...
      (let [states (get-in editables [app-id :states])
            zoom (get-in editables [app-id :svg-ctrl :zoom] (inter/fit-to-content-zoom states))
                              ...
                              (inter/update-connector-hover!
                                ...
                                (fn [sid]
                                  (let [[bx by] (get-in states [sid :diagram :corner] [0 0])]
                                    [bx by (geom/state-width sid) geom/STATE-H]))))
            on-pointer-up (fn [e]
                            (when (:drag @state)
        ...
        [:div
         ...
         (when connecting
           (let [[mx my] connecting
                 ...
                 [cx cy] (geom/compute-exit2point {:x x :y y :h geom/STATE-H :w (geom/state-width id)} [mx my])
